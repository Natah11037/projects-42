...
class SpellCard(Card):
    ...
    def play(self, game_state: dict) -> dict:
        ...
        game_state["effect"] = self.resolve_effect(
            game_state.get("enemy_deck", []))
        return {
            "card_played": self.name,
            "mana_used": self.cost,
            **self.resolve_effect(game_state.get("enemy_deck", [])),
        }

    def resolve_effect(self, targets: list) -> dict:
        targets = [
            target for target in targets if isinstance(target, CreatureCard)
        ]
        if targets:
            if "heal" in self.effect_type:
                return {"effect": f"{self.effect_type} every allies"}
            elif "damage" in self.effect_type:
                return {
                    "effect": f"deal {round(self.cost / 6)} to "
                ...
                return {"effect": f"giving {self.effect_type} to every allies"}
            else:
                return {"effect": None}
        return {"effect": "no creature targets"}
